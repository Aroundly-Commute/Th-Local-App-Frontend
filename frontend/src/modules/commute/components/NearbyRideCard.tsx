...
export const NearbyRideCard: React.FC<NearbyRideCardProps> = ({
  ride,
  t,
  onPress,
  testID,
    ...
    <TouchableOpacity testID={testID} onPress={onPress} activeOpacity={0.7}
      ...
      <View style={styles.row}>
        ...
        <View style={{ flex: 1, marginLeft: 12 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            <Text style={[styles.driver, { color: t.textPrimary }]} numberOfLines={1}>{driverName}</Text>
            {driverGender && (
          ...
          <View style={[styles.row, { gap: 6, marginTop: 2 }]}>
            ...
            <Text style={[styles.meta, { color: t.textSecondary }]}>{driverRating.toFixed(1)}</Text>
            <Text style={[styles.meta, { color: t.textTertiary }]}>·</Text>
            <Text style={[styles.meta, { color: t.textSecondary }]}>{seatsAvailable} seats</Text>
          </View>
        </View>
      ...
      {!compact && vehicle?.model && (
        <View style={[styles.row, { justifyContent: 'flex-end' }]}>
          <Text style={[styles.meta, { color: t.textTertiary }]}>{vehicle.make} {vehicle.model}</Text>
        </View>
      )}
    </TouchableOpacity>
  );
