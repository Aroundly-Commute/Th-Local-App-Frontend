...
export default function CommuteDashboard() {
  ...
  if (enableRideSharing) {
    actions.push(
      { icon: MapPin, label: 'Find Ride', onPress: () => router.push('/commute/search') },
      { icon: Users, label: 'Offer Ride', onPress: () => router.push({ pathname: '/commute/search', params: { mode: 'offer' } }) },
      { icon: Calendar, label: 'Schedule', onPress: () => router.push('/commute/rides') }
    );
  }
  if (enableParking) {
    actions.push(
      { icon: MapPin, label: 'Parking', onPress: () => router.push('/parking') }
    );
  }
